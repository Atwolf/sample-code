...
SubAgentRegistry = Union[List[ADKAgent], Dict[str, ADKAgent]]
AgentSelector = Callable[
    [RunAgentInput, Request, Dict[str, ADKAgent]],
    Union[ADKAgent, None, Coroutine[Any, Any, Optional[ADKAgent]]],
]
...
def _agent_registry_key(agent: ADKAgent) -> str:
    """Return a deterministic key for a list-provided sub-agent."""
    raw_agent = getattr(agent, "_adk_agent", None)
    name = getattr(raw_agent, "name", None)
    if not isinstance(name, str) or not name:
        raise ValueError(
            "List-based sub_agents require each ADKAgent to wrap an ADK agent "
            "with a non-empty 'name'. Use dict[str, ADKAgent] for explicit IDs."
        )
    return name


def _normalize_sub_agents(
    sub_agents: Optional[SubAgentRegistry],
) -> Dict[str, ADKAgent]:
    if sub_agents is None:
        return {}
    if isinstance(sub_agents, dict):
        return dict(sub_agents)

    normalized: Dict[str, ADKAgent] = {}
    for sub_agent in sub_agents:
        key = _agent_registry_key(sub_agent)
        if key in normalized:
            raise ValueError(
                f"Duplicate sub_agent key '{key}'. Use dict[str, ADKAgent] "
                "when multiple agents share the same ADK agent name."
            )
        normalized[key] = sub_agent
    return normalized
...
async def _select_agent_for_request(
    *,
    ...
    request: Request,
    sub_agents: Dict[str, ADKAgent],
    select_agent_from_state: Optional[AgentSelector],
) -> ADKAgent:
    if select_agent_from_state is None:
        return default_agent

    selected_agent = select_agent_from_state(input_data, request, sub_agents)
    if inspect.isawaitable(selected_agent):
        selected_agent = await selected_agent

    return selected_agent or default_agent
...
def add_adk_fastapi_endpoint(
    ...
    sub_agents: Optional[SubAgentRegistry] = None,
    select_agent_from_state: Optional[AgentSelector] = None,
):
    ...
    Args:
        ...
        sub_agents: Optional selectable sub-agent registry. Dict keys are used
            as the stable selector contract. List entries are keyed by the
            wrapped ADK agent's name.
        select_agent_from_state: Optional request-scoped selector. It receives
            the post-extractor RunAgentInput, FastAPI Request, and normalized
            sub-agent registry. Return an ADKAgent to run that agent for this
            request, or None to run the primary agent.
    ...
    configured_sub_agents = (
        sub_agents if sub_agents is not None else getattr(agent, "_sub_agents", None)
    )
    configured_select_agent_from_state = (
        select_agent_from_state
        if select_agent_from_state is not None
        else getattr(agent, "_select_agent_from_state", None)
    )
    normalized_sub_agents = _normalize_sub_agents(configured_sub_agents)
    ...
    async def adk_endpoint(input_data: RunAgentInput, request: Request):
        ...
        * Any other content type negotiated by ``EventEncoder`` (e.g. a future
        ...
        selected_agent = await _select_agent_for_request(
            default_agent=agent,
            input_data=input_data,
            request=request,
            sub_agents=normalized_sub_agents,
            select_agent_from_state=configured_select_agent_from_state,
        )

        if content_type == "text/event-stream":
            return EventSourceResponse(_sse_stream(selected_agent, input_data))
        return StreamingResponse(
            _legacy_stream(selected_agent, input_data, encoder),
...
def create_adk_app(
    ...
    Args:
        ...
        extract_state_from_request: Optional async function to extract values mapped from the request into state.
            ...
            The RunAgentInput is provided so conflicts can be identified and resolved appropriately.
            Cannot be used with extract_headers.
        sub_agents: Optional selectable sub-agent registry.
        select_agent_from_state: Optional request-scoped selector.
